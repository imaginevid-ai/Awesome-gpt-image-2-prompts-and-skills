...
export function scoreStoredTwitterPrompt(prompt: StoredTwitterPrompt): QualityResult {
  ...
  if (prompt.sourceMeta?.model_evidence !== "gpt-image-2") {
    hardFailures.push("missing verified GPT Image 2 model evidence");
  }
...
export function scoreTwitterCandidate(candidate: TwitterCandidate): QualityResult {
  ...
  const hasGptImage2Evidence = /\bgpt[-\s]*image\s*2\b/i.test(relevanceText);
  if (hasPromptSignal) {
    score += 30;
  ...
  if (!hasGptImage2Evidence) {
    reasons.push("missing explicit GPT Image 2 evidence; manual review required");

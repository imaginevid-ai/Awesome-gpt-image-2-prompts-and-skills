...
const REPO = "imaginevid-ai/Awesome-gpt-image-2-prompts-and-skills";
const REPO_URL = `https://github.com/${REPO}`;
const GPT_IMAGE_2_COLLECTION_URL = REPO_URL;
const OPENAI_IMAGE_GUIDE_URL = "https://developers.openai.com/api/docs/guides/image-generation";
const OPENAI_IMAGE_API_REFERENCE_URL = "https://developers.openai.com/api/reference/resources/images";

export function getGptImage2CollectionUrl(_locale: string): string {
  return GPT_IMAGE_2_COLLECTION_URL;
}

export function getPromptCtaLabel(locale: string): string {
  return locale === "en" ? "Use this prompt on GPT Image 2 Library" : `${t("tryItNow", locale)} · GPT Image 2 Library`;
}
...
function generateHeader(locale: string): string {
  const coverImage = "public/images/gpt-image-2-prompt-skills-cover.png";

  return `<a href="${REPO_URL}">
  <img src="${coverImage}" alt="GPT Image 2 Prompt Skills" width="100%" />
</a>
...
**[${t("browseGallery", locale)}](${getGptImage2CollectionUrl(locale)})**
...
function generatePromptSection(
  ...
  md += `**[${getPromptCtaLabel(locale)}](${getGptImage2CollectionUrl(locale)})**\n\n`;
  md += "---\n\n";
...
**${sourceLabel}:** [OpenAI image generation guide](${OPENAI_IMAGE_GUIDE_URL}) · [OpenAI Image API reference](${OPENAI_IMAGE_API_REFERENCE_URL}) · [GPT Image 2 prompt collection](${getGptImage2CollectionUrl(locale)})
...
function sourceBackedNote(locale: string): string {
  if (locale === "zh") {
    return "本仓库只使用本仓库链接作为项目入口；模型能力来源链接指向 OpenAI 官方文档。";
  }
  if (locale === "zh-TW") {
    return "本倉庫只使用本倉庫連結作為專案入口；模型能力來源連結指向 OpenAI 官方文件。";
  }
  return "Project links stay inside this repository; model capability sources point to official OpenAI documentation.";
}
...
- [GPT Image 2 Prompt Library](${REPO_URL})
${creatorCredits}
...
**[${t("viewInGallery", locale)}](${getGptImage2CollectionUrl(locale)})** •
